<template>
  <div class="container">
    <div class="tools">
      <el-row>
        <el-col :span="3">
          <el-input v-model="siftName" clearable maxlength="20" placeholder="菜单名称"></el-input>
        </el-col>
        <el-col :span="3" :offset="1">
          <el-button icon="search" type="primary" @click="getDataList()">查询</el-button>
          <el-button icon="plus" type="primary" @click="dialogForm = true">添加</el-button>
        </el-col>
      </el-row>
    </div>
    <div class="table">
          ...
          <template v-slot="scope">
            <el-dropdown :show-timeout="50">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item>
                    <el-button icon="edit" text class="waring" @click="handleEdit(scope.row)">修改</el-button>
                  </el-dropdown-item>
                  <el-dropdown-item>
    ...
    <el-dialog title="菜单管理" v-model="dialogForm" draggable :close-on-click-modal="false" @close="beforeClose(refForm)">
      <el-form :model="form" :rules="rules" ref="refForm" label-width="80px" label-position="left">
        ...
        <el-form-item prop="parent" label="父级菜单">
          <el-select v-model="form.parent" placeholder="父级菜单">
            <el-option v-for="menu in menus" :key="menu.id" :value="menu.id" :label="menu.label"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item prop="sort" label="优先级">
          <el-input-number v-model="form.sort" placeholder="优先级"></el-input-number>
        </el-form-item>
      </el-form>
      <template #footer>
...
let dataList = reactive<Menus[]>([])
let menus = reactive<Menus[]>([]);
const getDataList = () => {
  getMenu({ size: size.value, current: current.value, siftName: siftName.value }).then((res: any) => {
    if (res.code === 200) {
      dataList = res.data.data;
      total.value = res.data.total;
      menus = JSON.parse(JSON.stringify(dataList))
      menus.push({ id: 0, label: '根目录', sort: 0 })
    }
  })
}
